...
@get_users_router.get("/users", response_model=FinanceUserListResponse, description="Получить список пользователей")
async def get_users(
...
@get_users_router.get("/users/{user_id}", response_model=FinanceUserModel, description="Получить пользователя по id")
...
@get_users_router.get("/users/{user_id}/details", response_model=FinanceUserDetailsResponse, description="Получить пользователя с вложенными связями")
