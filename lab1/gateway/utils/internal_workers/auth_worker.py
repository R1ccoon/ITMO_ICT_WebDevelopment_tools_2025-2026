...
class AuthNamespace:
    ...
    @staticmethod
    async def _authenticate_user(email: str, raw_password: str) -> User:
        user: User = await database_worker.custom_orm_select(
            cls_from=User, where_params=[User.email == email], return_unpacked=True
        )
    ...
    async def get_current_user(token: str) -> User:
        ...
        user = await database_worker.custom_orm_select(
            cls_from=User,
            where_params=[User.email == payload.get("sub")],
    ...
    @staticmethod
    async def verify_route_access(request: Request, token: str) -> User:
